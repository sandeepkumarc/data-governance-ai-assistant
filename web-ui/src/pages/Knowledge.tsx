...
import { KnowledgeBaseGuide } from "../components/KnowledgeBaseGuide";
import type { KbNlUpdateResult, KbSection, KnowledgeSectionVerification } from "../types";
import { KnowledgeSectionVerificationPanel } from "../components/KnowledgeSectionVerification";
...
export function KnowledgePage() {
  const { governance, apiKey, usesOfflineData, refreshStatus } = useApp();
  ...
  const [verification, setVerification] = useState<KnowledgeSectionVerification | null>(null);
  ...
  function selectSection(section: KbSection) {
    ...
    setError("");
    setVerification(section.verification ?? null);
  }

  async function runVerification(title: string, text: string) {
    setVerifyLoading(true);
    setError("");
    try {
      const result = await governance.verifyKbSection(
        { title, text, retrieval_mode: "tfidf" },
        apiKey || undefined,
      );
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        ...
        setVerification(updated.verification ?? null);
    ...
    <div>
      ...
      {loading && sections.length === 0 ? (
        ...
        <div className="grid gap-6 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3" ref={editorRef}>
            {isNew || selectedTitle ? (
              <Card
                title={isNew ? "New section" : "Edit section"}
                className={isNew ? "ring-2 ring-indigo-300 dark:ring-indigo-700" : undefined}
              >
                <div className="space-y-4">
                  {isNew && (
                    <p className="rounded-lg bg-indigo-50 px-3 py-2 text-xs text-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-200">
                      Draft a new policy section below, then click <strong>Create section</strong> or{" "}
                  ...
                  <div className="flex flex-wrap gap-3 border-t border-slate-100 pt-4 dark:border-slate-700">
                    ...
                    <Button
                      variant="secondary"
                      type="button"
                      disabled={verifyLoading || !editTitle.trim()}
                      onClick={() => void runVerification(editTitle.trim(), editText)}
                    >
                      {verifyLoading ? <Spinner className="h-4 w-4" /> : <Search className="h-4 w-4" />}
                      Test retrieval
                    </Button>
                    {!isNew && selectedTitle && (
                  ...
                  {verification && (
                    <KnowledgeSectionVerificationPanel
                      verification={verification}
                      onRerun={() => void runVerification(editTitle.trim(), editText)}
                      rerunning={verifyLoading}
                    />
                  )}

                  {!verification && !isNew && selectedTitle && (
                    <div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900/50">
                      <p className="text-xs font-medium text-slate-500">Retrieval confidence</p>
                      <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
                        Save changes or click <strong>Test retrieval</strong> to generate sample data and
                        verify this section is cited during analysis.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <AdminSection
        title="Administration — guides & natural language"
        description="How to write policy sections and bulk-update the knowledge base with plain English."
        open={showAdmin}
      ...
      >
        ...
        <div className="rounded-lg border border-indigo-100 dark:border-indigo-900">
          ...
          >
            ...
            Update knowledge base in natural language
          ...
          {showNlPanel && (
            <div className="space-y-4 border-t border-indigo-100 px-4 py-4 dark:border-indigo-900">
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Describe policy changes in plain English — preview first, then apply.
              </p>
              <textarea
                value={nlInstruction}
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Button disabled={nlLoading || !nlInstruction.trim()} onClick={() => handleNlUpdate(true)}>
              ...
              {nlResult && (
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/50">
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{nlResult.summary}</p>
                  <p className="mt-1 text-xs text-slate-500">
                    {nlResult.applied ? "Applied" : "Preview only"} · {nlResult.changes.length} change(s)
                  </p>
