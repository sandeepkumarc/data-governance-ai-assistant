...
import { DEFAULT_CREDENTIALS } from "../data/sampleData";
import { PRODUCT_NAME, PRODUCT_TAGLINE } from "../lib/product";
import { Button, Spinner } from "../components/ui";

export function LoginPage() {
  const { user, login, loginOffline, theme, toggleTheme, setOfflineMode } = useApp();
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      if (!ok) {
        setError(
          `Invalid credentials. Use ${DEFAULT_CREDENTIALS.email} / ${DEFAULT_CREDENTIALS.password}`,
        );
      }
      setLoading(false);
    }, 600);
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 px-4">
      ...
      <div className="relative w-full max-w-md animate-fade-in">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-indigo-200/80">{PRODUCT_TAGLINE}</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-3 text-center text-xs text-slate-500">
            Explore the platform tour after sign-in.
          </p>

          <p className="mt-4 text-center text-[11px] text-slate-500">
            Default sign-in:{" "}
            <code className="text-indigo-300">{DEFAULT_CREDENTIALS.email}</code> /{" "}
            <code className="text-indigo-300">{DEFAULT_CREDENTIALS.password}</code>
          </p>
        </div>
