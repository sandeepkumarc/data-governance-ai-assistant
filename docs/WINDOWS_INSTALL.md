# AI-Assisted Data Governance — Windows Installation Guide

Step-by-step setup for AI-Assisted Data Governance on a **Windows 10/11** laptop.

**Time required:** 45–60 minutes (first time)  
**Fallback:** Use **AI-Assisted Data Governance (offline)** in the UI if install issues occur on presentation day (no backend needed).
...
| What | How |
|------|-----|
| **UI (React)** | Docker container — `web-ui` |
...
**Terminal 2 — Web UI (Docker = Node.js)**
...
> **Ollama note:** With full Docker, Ollama on the Windows host is reached at `http://host.docker.internal:11434` if you add that env to the backend service later. For presentation day, **Option A** is simpler when Ollama runs natively.
...
# View UI logs
docker compose -f docker-compose.web-ui.yml logs -f web-ui
```
...
Ollama runs as a Windows service after install. Keep it running during live analysis.

**Skip Ollama if:** You will use **AI-Assisted Data Governance (offline)** or **RAG-only mode** (`no_llm: true`).
...
## Step 6 — Start the web UI (Terminal 2)
...
```powershell
cd $HOME\Documents\data-gov-ai-assistant\web-ui
npm install
npm run dev
```
...
## Step 8 — Pre-flight smoke test (5 min)
...
## Executive presentation day checklist
...
- [ ] Terminal 2: web UI running (Docker `docker-compose.web-ui.yml` **or** `npm run dev` on :5173)
- [ ] Ollama app running (if using LLM/vector)
- [ ] Browser bookmark: http://localhost:5173
- [ ] Backup: know how to click **Work offline**
- [ ] Sample file ready: `backend\sample_metadata.csv`
- [ ] Deck open: `docs\EXECUTIVE_OVERVIEW.md`
...
Ensure backend is on port 8000 and use `docker-compose.web-ui.yml` (proxies to `host.docker.internal:8000`).
...
```powershell
cd web-ui
docker build -t govassist-web-ui .
docker run --rm -p 4173:4173 govassist-web-ui npm run build && npm run preview -- --host 0.0.0.0 --port 4173
```
...
- Executive deck: `docs\EXECUTIVE_OVERVIEW.md`
- Project continuity: `docs\PROJECT_HANDOFF.md`
- Web UI details: `web-ui\README.md`
