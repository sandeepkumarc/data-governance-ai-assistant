# AI-Assisted Data Governance — Executive Overview
...
**Live UI:** AI-Assisted Data Governance UI at `http://localhost:5173` (offline mode works without backend)

---

## Slide 1 — Title

**AI-Assisted Data Governance: AI-Assisted Data Governance**
...
- No cloud LLM optional for local LLM (local Ollama optional)
...
## Slide 3 — What AI-Assisted Data Governance does (one sentence)
...
```text
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────────┐
│  AI-Assisted Data Governance UI    │────▶│  FastAPI backend │────▶│  governance.db      │
│  (React web app)   │     │  + RAG engine    │     │  (definitions,      │
└─────────────────┘     └────────┬─────────┘     │   lineage, audit)   │
                                 │               └─────────────────────┘
                                 ▼
                        ┌──────────────────┐
...
## Slide 6 — Live walkthrough (15 min script)
...
**Fallback:** If live backend fails → **Work offline** on login (embedded data).

---

## Slide 7 — Sample data story (narrative)

Use **`backend/clinical_ehr_patients.csv`** — a realistic table export with sample patient rows:
...
| IT install friction | Offline mode; Windows install guide in repo |
...
Architecture supports provider swap; the default deployment uses local Ollama.

**"How accurate is it?"**  
Pilot measures steward accept/edit/reject rates — target 70%+ accept with minor edits.

**"What does IT need to install?"**  
Python, Node, optional Ollama — see `docs/WINDOWS_INSTALL.md`.

---

## Slide 15 — Close

**AI-Assisted Data Governance turns governance policy into scalable metadata enrichment — with humans in control.**
...
| AI-Assisted Data Governance (offline) | "Work offline" button (no install needed) |
